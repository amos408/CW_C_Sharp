...
public class Kata
{
  public static string Encrypt(string text, int n)
  {
      if (string.IsNullOrEmpty(text) || n <= 0) return text;
            while (n-- > 0)
            {
                text = string.Concat(text.Where((v, b) => (b % 2 == 1)).Concat(text.Where((v, b) => (b % 2 == 0))));
            }
            return text;
  }
  public static string Decrypt(string encryptedText, int n)
  {
    if (string.IsNullOrEmpty(encryptedText) || n <= 0) return encryptedText;
            while (n-- > 0)
            {
                ...
                encryptedText = string.Concat(Enumerable.Range(0, encryptedText.Length).Select(b => b % 2 == 0 ? rightpart[b / 2] : leftpart[b / 2]));
            }
